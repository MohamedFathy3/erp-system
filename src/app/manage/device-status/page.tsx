...
export default function Page() {
  ...
  const { data: brandsData, isLoading, error } = useQuery<ApiResponse>({
    queryKey: ['device-status', currentPage, showingDeleted, filters, orderBy, orderByDirection],
    queryFn: async () => {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/device-status/index`, {
        method: 'POST',
...
staleTime: 5 * 60 * 60 * 1000, 
  ...
  const saveBrandMutation = useMutation({
    mutationFn: async (brandData: Brand) => {
      let url;
      let method;

      if (brandData.id) {
        url = `${process.env.NEXT_PUBLIC_API_URL}/device-status/${brandData.id}`;
        method = "PATCH";
      } else {
        url = `${process.env.NEXT_PUBLIC_API_URL}/device-status`;
        method = "POST";
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(errorText || "Failed to save device-status");
      }
    ...
    onSuccess: () => {
      ...
      toast.success(editingBrand ? 'device-status updated successfully!' : 'device-status created successfully!');
      setOpen(false);
  ...
  const deleteBrandMutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/device-status/delete`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
  ...
  const bulkRestoreMutation = useMutation({
    mutationFn: async (ids: number[]) => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/device-status/restore`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ Items: ids }),
      });
  ...
  const handleSave = async (
    ...
    saveBrandMutation.mutate(brandData as Brand);
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Are you sure you want to delete ${selectedItems.size} device-status(s)?`)) {
        ...
        {open && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-md p-6 relative">
              <button 
                onClick={() => { setOpen(false); setEditingBrand(null); }} 
              ...
              <h2 className="text-2xl font-semibold mb-6 text-gray-900 dark:text-gray-100">
                {editingBrand ? "Edit device-status" : "Add device-status"}
              </h2>
              <form className="space-y-4" onSubmit={handleSave}>
                <Input 
                  name="name" 
                  placeholder="Brand device_status" 
                ...
                >
                  {saveBrandMutation.isPending ? "Saving..." : (editingBrand ? "Update" : "Create")}
                </Button>
              </form>
            </div>
          </div>

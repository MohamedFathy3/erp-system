...
export default function Page() {
  ...
  const { data: categoriesData, error } = useQuery<ApiResponse>({
    queryKey: ['categories', currentPage, showingDeleted, filters, orderBy, orderByDirection],
    queryFn: async () => {
      ...
      if (filters.type_id) {
        payload.filters.type_id = filters.type_id;
      }

      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/category/index`, {
        method: 'POST',
  ...
  const isValidTimeFormat = (time: string) => {
    const timeRegex = /^([0-9]|[0-9][0-9]):([0-5][0-9])$/;
  ...
  const saveCategoryMutation = useMutation({
    mutationFn: async (categoryData: Category) => {
      ...
      if (categoryData.id) {
        url = `${process.env.NEXT_PUBLIC_API_URL}/category/${categoryData.id}`;
        method = "PATCH";
      } else {
  ...
  const bulkRestoreMutation = useMutation({
    mutationFn: async (ids: number[]) => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/category/restore`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ Items: ids }),
      });
  ...
  const handleSave = async (
    ...
    const timeInCorrectFormat = `${timeInput}:00`;
    ...
    const categoryData = {
      ...
      type_id: 8,
      time: timeInCorrectFormat,
    };

    saveCategoryMutation.mutate(categoryData as Category);
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Are you sure you want to delete ${selectedItems.size} category?`)) {
    ...
    <MainLayout>
      <div className="space-y-8 p-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Ticket ETA</h1>
          </div>
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-800">
              <tr>
                ...
                {["ID", "Name", "Type ID", "Time", "Actions"].map((header) => (
                  <th key={header} className="px-6 py-3 text-center text-gray-700 dark:text-gray-300 font-medium uppercase tracking-wider">
                    <div className="flex items-center justify-center gap-1 cursor-pointer hover:text-indigo-600"
                      onClick={() => {
                        if (orderBy === header.toLowerCase().replace(' ', '_')) {
                          setOrderByDirection(orderByDirection === 'asc' ? 'desc' : 'asc');
                        } else {
                          setOrderBy(header.toLowerCase().replace(' ', '_'));
              ...
              {data.length ? (
                data.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                    ...
                    <td className="px-6 py-4 text-gray-700 dark:text-gray-300">{item.type_id}</td>
                    <td className="px-6 py-4 text-gray-700 dark:text-gray-300">{item.time}</td>
                    ...
                    <td className="px-6 py-4 flex justify-center gap-2">
                      <Button
                        ...
                        onClick={() => {
                          ...
                          setTimeInput(formatTimeForDisplay(item.time));
                          setOpen(true);
                        }}
                      >
                        Edit
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                    No categories found {Object.values(filters).some(f => f) ? 'matching your filters' : ''}
                  </td>
                </tr>
              )}
            </tbody>
              ...
              <form className="space-y-4" onSubmit={(e) => handleSave(e, true)}>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    name="time"
                    type="text"
                    placeholder="Enter time (e.g., 99:55)"
                    value={timeInput}
                    onChange={(e) => setTimeInput(e.target.value)}
                    required
                    className="rounded-xl dark:bg-gray-800 dark:text-gray-100 dark:border-gray-600 border-2 border-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 px-4 py-2 text-sm"
                  />
                  <p className="text-xs text-gray-500">Format: Hours:Minutes (e.g., 99:55)</p>
                </div>

                <Input
                  name="short_description"
                  placeholder="Short Description (Optional)"
                  defaultValue={editingCategory?.short_description || ""}
                  className="rounded-xl dark:bg-gray-800 dark:text-gray-100"
                />
                ...
                <div className="flex gap-4">
                  ...
                  >
                    {saveCategoryMutation.isPending ? "Saving..." : "Save & Keep Open"}
                  </Button>
                  ...
                  <Button
                    ...
                    disabled={saveCategoryMutation.isPending}
                  >
                    {saveCategoryMutation.isPending ? "Saving..." : (editingCategory ? "Update & Close" : "Create & Close")}
                  </Button>
                </div>
              </form>
            </div>

...
export default function Page() {
  ...
  const handleSave = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingCategory) {
        // Update existing category - PATCH to /graphic-card/{id}
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/graphic-card/${editingCategory.id}`, {
          method: "PATCH",
        ...
        if (!res.ok) {
          throw new Error("Failed to update graphic-card");
        }
      } else {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/graphic-card`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(categoryData),
        });
        ...
        if (!res.ok) {
          throw new Error("Failed to create graphic-card");
        }
      }
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Are you sure you want to delete ${selectedItems.size} graphic-cards?`)) {
      try {
  ...
  return (
    <MainLayout>
      <div className="space-y-8 p-6">
        ...
        {/* Search */}
        <Input
          placeholder="Search graphic-cards..."
          value={search}

...
export default function Page() {
  ...
  const handleSave = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingCategory) {
        // Update existing category - PATCH to /position/{id}
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/position/${editingCategory.id}`, {
          method: "PATCH",
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/position`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(categoryData),
        });
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Are you sure you want to delete ${selectedItems.size} position(s)?`)) {
      try {

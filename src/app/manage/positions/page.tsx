...
export default function Page() {
  ...
  async function fetchCategories(page = 1) {
    try {
      setLoading(true);
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/position?page=${page}`);
      if (!res.ok) {
      ...
      setLoading(false);
      toast.success('Fetched position successfully!');
    }
  }
 ...
 const handleSave = async (
  ...
  try {
    ...
    if (editingCategory) {
      // Update category
      res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/position/${editingCategory.id}`, {
        method: "PATCH",
      ...
      // Create new category
      res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/position`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(categoryData),
      });
 ...
 const handleDelete = async (id: number) => {
  if (confirm("Are you sure you want to delete this position?")) {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/position/delete`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
...
async function bodyfilter() {
 ...
 const payload = {
  ...
  deleted: false,
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/position/index`, {
      method: 'POST',
      headers: {
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Are you sure you want to delete ${selectedItems.size} position?`)) {
      try {
...
const handleBulkRestore = async () => {
  ...
  if (confirm(`${selectedItems.size}  you want to restore?`)) {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/position/restore`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ Items: ids }),
      });
    ...
    <MainLayout>
      <div className="space-y-8 p-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Position</h1>
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
    <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-md p-6 relative transform scale-95 animate-fadeIn">
      ...
      <h2 className="text-2xl font-semibold mb-6 text-gray-900 dark:text-gray-100">
        {editingCategory ? "Edit position" : "Add position"}
      </h2>

...
export default function Page() {
  ...
  const { data: processorsData, isLoading, error } = useQuery<ApiResponse>({
    queryKey: ['Device_Models', currentPage, showingDeleted, filters, orderBy, orderByDirection],
    queryFn: async () => {
      ...
      if (filters.brand) {
        payload.filters.brand = filters.brand;
      }

      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/device-model/index`, {
        method: 'POST',
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch Device_Models');
      }
  ...
  const data = processorsData?.data || [];
  const pagination = processorsData?.meta || {
    current_page: 1,
    last_page: 1,
    per_page: 15,
    total: 0,
  ...
  // استخدام React Query للطفرات (Mutations)
  const saveProcessorMutation = useMutation({
    mutationFn: async (processorData: Processor) => {
      let url;
      let method;

      if (processorData.id) {
        url = `${process.env.NEXT_PUBLIC_API_URL}/device-model/${processorData.id}`;
        method = "PATCH";
      } else {
        url = `${process.env.NEXT_PUBLIC_API_URL}/device-model`;
        method = "POST";
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(errorText || "Failed to save Device_Models");
      }
    ...
    onSuccess: () => {
      ...
      toast.success(editingProcessor ? 'Device_Models updated successfully!' : 'Device_Models created successfully!');
      setOpen(false);
  ...
  const deleteProcessorMutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/device-model/delete`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        throw new Error("Failed to delete Device_Models");
      }
  ...
  const bulkRestoreMutation = useMutation({
    mutationFn: async (ids: number[]) => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/device-model/restore`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ items:[ids] }),
      });
  ...
  const handleSave = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const processorData = {
      ...(editingProcessor && { id: editingProcessor.id }),
      name: formData.get("name") as string,
      brandId: selectedBrandId
    };

    saveProcessorMutation.mutate(processorData as Processor);
  };

  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this Device Models?")) {
      deleteProcessorMutation.mutate(id);
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Are you sure you want to delete ${selectedItems.size} Device Models(s)?`)) {
    ...
    <MainLayout>
      <div className="space-y-8 p-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Device Models</h1>
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-800">
              <tr>
                ...
                {["ID", "Name", "Brand", "Actions"].map((header) => (
...
<td className="px-6 py-4 text-gray-700 dark:text-gray-300">{item.brand?.id || ''}</td>
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                    No device models found {Object.values(filters).some(f => f) ? 'matching your filters' : ''}
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {open && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-md p-6 relative">
              <button 
                onClick={() => { setOpen(false); setEditingProcessor(null); setSelectedBrandId(null); }} 
              ...
              <h2 className="text-2xl font-semibold mb-6 text-gray-900 dark:text-gray-100">
                {editingProcessor ? "Edit Device Model" : "Add Device Model"}
              </h2>
              <form className="space-y-4" onSubmit={handleSave}>
                <Input 
                  name="name" 
                  placeholder="Device Model Name" 
                  defaultValue={editingProcessor?.name || ""} 
                  required 
                ...
                <Button 
                  ...
                  disabled={saveProcessorMutation.isPending}
                >
                  {saveProcessorMutation.isPending ? "Saving..." : (editingProcessor ? "Update" : "Create")}
                </Button>
              </form>
            </div>
          </div>

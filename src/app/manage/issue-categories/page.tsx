...
export default function Page() {
  ...
  const { data: memoryData, isLoading, error } = useQuery<ApiResponse>({
    queryKey: ['type', currentPage, showingDeleted, filters, orderBy, orderByDirection],
    queryFn: async () => {
  ...
  const saveMemoryMutation = useMutation({
    mutationFn: async (memoryData: Memory) => {
      ...
      if (memoryData.id) {
        url = `${process.env.NEXT_PUBLIC_API_URL}/type/${memoryData.id}`;
        method = "PATCH";
      } else {
  ...
  const requestData = {
    ...
    onSuccess: () => {
      ...
      toast.success(editingMemory ? 'type updated successfully!' : 'type created successfully!');
      setOpen(false);

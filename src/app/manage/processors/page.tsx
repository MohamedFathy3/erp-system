...
export default function Page() {
  ...
  async function fetchCategories(page = 1) {
    try {
      setLoading(true);
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/processor?page=${page}`);
      if (!res.ok) {
 ...
 const handleSave = async (
  ...
  try {
    ...
    if (editingCategory) {
      // Update category
      res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/processor/${editingCategory.id}`, {
        method: "PATCH",
      ...
      // Create new category
      res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/processor`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(categoryData),
      });
 ...
 const handleDelete = async (id: number) => {
  if (confirm("Are you sure you want to delete this category?")) {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/processor/delete`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
...
async function bodyfilter() {
 ...
 const payload = {
  ...
  deleted: false,
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/processor/index`, {
      method: 'POST',
      headers: {
...
const handleBulkRestore = async () => {
  ...
  if (confirm(`${selectedItems.size}  you want to restore?`)) {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/processor/restore`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ Items: ids }),
      });
...
{showFilter && (
  <div className="w-full bg-gray-100 dark:bg-gray-700 p-6 rounded-md space-y-6">
    ...
    <div className="flex flex-wrap items-center gap-4">
     ...
     <input
      type="text"
      name={orderBy}  // اسم الـ input يتغير مع الاختيار
      placeholder={`Enter ${orderBy}...`} // placeholder يتغير عشان يوضح
      value={filterValue}
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
    <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-md p-6 relative transform scale-95 animate-fadeIn">
      ...
      <h2 className="text-2xl font-semibold mb-6 text-gray-900 dark:text-gray-100">
        {editingCategory ? "Edit Processor" : "Add Processor"}
      </h2>

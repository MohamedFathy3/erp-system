...
export default function Page() {
  ...
  const { data: processorsData, isLoading, error } = useQuery<ApiResponse>({
    queryKey: ['processors', currentPage, showingDeleted, filters, orderBy, orderByDirection],
    queryFn: async () => {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/processor/index`, {
        method: 'POST',
...
staleTime: 5 * 60 * 60 * 1000,
  ...
  const saveProcessorMutation = useMutation({
    mutationFn: async (processorData: Processor) => {
      ...
      if (processorData.id) {
        url = `${process.env.NEXT_PUBLIC_API_URL}/processor/${processorData.id}`;
        method = "PATCH";
      } else {
        url = `${process.env.NEXT_PUBLIC_API_URL}/processor`;
        method = "POST";
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(errorText || "Failed to save processor");
      }
    ...
    onSuccess: () => {
      ...
      toast.success(editingProcessor ? 'Processor updated successfully!' : 'Processor created successfully!');
      setOpen(false);
  ...
  const deleteProcessorMutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/processor/delete`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async (ids: number[]) => {
      ...
      if (!response.ok) {
        throw new Error("Failed to delete processors");
      }
  ...
  const bulkRestoreMutation = useMutation({
    mutationFn: async (ids: number[]) =>{
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/processor/restore`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ Items: ids }),
      });
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Are you sure you want to delete ${selectedItems.size} processor(s)?`)) {
    ...
    <MainLayout>
      <div className="space-y-8 p-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Processors</h1>
          </div>
              ...
              {data.length ? (
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                    No processors found {Object.values(filters).some(f => f) ? 'matching your filters' : ''}
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {open && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-md p-6 relative">
              <button 
                onClick={() => { setOpen(false); setEditingProcessor(null); }} 

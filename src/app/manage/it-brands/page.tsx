...
export default function Page() {
  ...
  async function fetchCategories(page = 1) {
    try {
      ...
      setLoading(false);
      toast.success('Fetched brand successfully!');
    }
  }
 ...
 const handleSave = async (
  ...
  try {
    ...
    if (editingCategory) {
      // Update category
      res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/brand/${editingCategory.id}`, {
        method: "PATCH",
      ...
      // Create new category
      res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/brand`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(categoryData),
      });
...
async function bodyfilter() {
 ...
 const payload = {
  ...
  deleted: false,
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/brand/index`, {
      method: 'POST',
      headers: {
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Are you sure you want to delete ${selectedItems.size} brands?`)) {
      try {
...
const handleBulkRestore = async () => {
  ...
  if (confirm(`${selectedItems.size}  you want to restore?`)) {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/brand/restore`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ Items: ids }),
      });

...
export default function Page() {
  ...
  const { data: brandsData, isLoading, error } = useQuery<ApiResponse>({
    queryKey: ['brands', currentPage, showingDeleted, filters, orderBy, orderByDirection],
    queryFn: async () => {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/brand/index`, {
        method: 'POST',
...
staleTime: 5 * 60 * 60 * 1000, 
  ...
  const saveBrandMutation = useMutation({
    mutationFn: async (brandData: Brand) => {
      ...
      if (brandData.id) {
        url = `${process.env.NEXT_PUBLIC_API_URL}/brand/${brandData.id}`;
        method = "PATCH";
      } else {
        url = `${process.env.NEXT_PUBLIC_API_URL}/brand`;
        method = "POST";
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(errorText || "Failed to save brand");
      }
  ...
  const deleteBrandMutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/brand/delete`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
  ...
  const bulkRestoreMutation = useMutation({
    mutationFn: async (ids: number[]) => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/brand/restore`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ Items: ids }),
      });
      ...
      <div className="space-y-8 p-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <Button
            variant="default"
              className=" text-white  transition-all  dark:hover:bg-blue-500 rounded-xl"
          ...
          <div className="w-full bg-gray-100 dark:bg-gray-700 p-6 rounded-md space-y-6">
            ...
            <div className="flex flex-col md:flex-row gap-4 pt-4">
              ...
              variant="default"
                ...
                className="w-full  text-white ansition-all rounded-md px-5 h-12 text-lg flex items-center justify-center gap-2 dark:bg-blue-600 dark:hover:bg-blue-500"
              >

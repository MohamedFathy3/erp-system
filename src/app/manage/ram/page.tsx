...
export default function Page() {
  ...
  async function fetchCategories(page = 1) {
    try {
      setLoading(true);
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/memory?page=${page}`);
      if (!res.ok) {
 ...
 const handleSave = async (
  ...
  try {
    ...
    if (editingCategory) {
      // Update category
      res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/memory/${editingCategory.id}`, {
        method: "PATCH",
      ...
      // Create new category
      res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/memory`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(categoryData),
      });
 ...
 const handleDelete = async (id: number) => {
  if (confirm("Are you sure you want to delete this memory?")) {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/memory/delete`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
...
async function bodyfilter() {
 ...
 const payload = {
  ...
  deleted: false,
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/memory/index`, {
      method: 'POST',
      headers: {
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Are you sure you want to delete ${selectedItems.size} memory?`)) {
      try {
...
const handleBulkRestore = async () => {
  ...
  if (confirm(`${selectedItems.size}  you want to restore?`)) {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/memory/restore`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ Items: ids }),
      });
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
    <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-md p-6 relative transform scale-95 animate-fadeIn">
      ...
      <h2 className="text-2xl font-semibold mb-6 text-gray-900 dark:text-gray-100">
        {editingCategory ? "Edit memory" : "Add memory"}
      </h2>

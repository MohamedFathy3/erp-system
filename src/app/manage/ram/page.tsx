...
export default function Page() {
  ...
  const handleSave = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingCategory) {
        // Update existing category - PATCH to /memory/{id}
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/memory/${editingCategory.id}`, {
          method: "PATCH",
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/memory`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(categoryData),
        });
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Are you sure you want to delete ${selectedItems.size} memory(s)?`)) {
      try {
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-800">
              <tr>
                ...
                {["ID", "type", "size", "Actions"].map((header) => (

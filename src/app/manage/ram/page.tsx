...
export default function Page() {
  ...
  const { data: memoryData,  } = useQuery<ApiResponse>({
    queryKey: ['memory', currentPage, showingDeleted, filters, orderBy, orderByDirection],
    queryFn: async () => {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/memory/index`, {
        method: 'POST',
  ...
  const saveMemoryMutation = useMutation({
    mutationFn: async (memoryData: Memory) => {
      ...
      if (memoryData.id) {
        url = `${process.env.NEXT_PUBLIC_API_URL}/memory/${memoryData.id}`;
        method = "PATCH";
      } else {
        url = `${process.env.NEXT_PUBLIC_API_URL}/memory`;
        method = "POST";
      }
    ...
    onSuccess: () => {
      ...
      toast.success(editingMemory ? 'Memory updated successfully!' : 'Memory created successfully!');
      setOpen(false);
  ...
  const deleteMemoryMutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/memory/delete`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
  ...
  const bulkRestoreMutation = useMutation({
    mutationFn: async (ids: number[]) => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/memory/restore`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ Items: ids }),
      });
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Are you sure you want to delete ${selectedItems.size} memory item(s)?`)) {
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-md p-6 relative">
              ...
              <form className="space-y-4" onSubmit={handleSave}>
                <Input 
                  name="size" 
                  placeholder="Memory Size (e.g., 8GB, 16GB)" 
                  defaultValue={editingMemory?.size || ""} 
                  required 
                ...
                <Input 
                  name="type" 
                  placeholder="Memory Type (e.g., DDR4, DDR5)" 
                  defaultValue={editingMemory?.type || ""} 
                  required 
                  className="rounded-xl dark:bg-gray-800 dark:text-gray-100" 
                />
